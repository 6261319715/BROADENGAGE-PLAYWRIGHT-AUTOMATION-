...
/**
 * Offers / Offer Form sub-module tests (M10.7).
 * Scope: /offer-form and /assets/offers (same remote) are accessible post-login and do not render blank screens.
 */
...
async function ensureLoggedIn(page: Page) {
  await page.route("**/api/v1/login", (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        success: true,
        data: { token: "fake-jwt-token-for-offers" },
      }),
    })
  );
...
async function expectNotBlankOffersSurface(page: Page) {
...
test.describe("Offers / Offer Form (M10.7)", () => {
  ...
  test("Offer form route loads (or fallback) post-login", async ({ page }) => {
    await ensureLoggedIn(page);
    await page.goto("/offer-form");
    await expect(page).not.toHaveURL(new RegExp(ROUTES.LOGIN));
    await expectNotBlankOffersSurface(page);
  });

  test("Assets offers route loads (or fallback) post-login", async ({ page }) => {
    await ensureLoggedIn(page);
    await page.goto("/assets/offers");
    await expect(page).not.toHaveURL(new RegExp(ROUTES.LOGIN));
    await expectNotBlankOffersSurface(page);
  });

  test("User can switch between offer-form and assets/offers without being kicked to login", async ({
    page,
  }) => {
    await ensureLoggedIn(page);
  ...
  test("Unauthenticated user is redirected to login for offers route", async ({ page }) => {
    await page.goto("/offer-form");
